...
function EducationAndExperience() {
  return (
    <section id="education-and-experience" className="mx-4 lg:mx-20 flex flex-col md:flex-row gap-4 md:gap-2">
      {/* =========== EDUCATION =========== */}
      <div className="w-full md:w-[80%]">
        {/* =========== EDUCATION TITLE =========== */}
        <h4 className="text-xl dark:text-white mb-4 font-bold flex gap-2 items-center">
          <FaLandmark className="text-xl text-[#C96868] dark:text-[#E78F81]" />
          Education
        </h4>
        ...
        {/* =========== CERTIFICATES TITLE =========== */}
        <h4 className="text-xl dark:text-white mt-12 font-bold flex gap-2 items-center">
          <PiCertificateFill className="text-2xl text-[#C96868] dark:text-[#E78F81]" />
          Certificates
        </h4>
        ...
        >
          <div className="hs-carousel relative overflow-hidden w-full h-48 bg-white dark:bg-[#1E3E62] rounded-lg">
              ...
              {user_info.certificates.map((cert, index) => {
                return (
                  <div className="hs-carousel-slide relative" key={index}>
                    ...
                    <div className="flex justify-center items-center h-full bg-gray-100 p-6 dark:bg-[#1E3E62]">
                      <div className="hs-tooltip [--placement:bottom] inline-block">
                        <a href={cert.link} target="_blank" rel="noreferrer" className="hs-tooltip-toggle text-center text-gray-800 dark:text-white hover:text-[#C96868] hover:dark:text-[#E78F81]">
                          {/* =========== CERTIFICATE NAME =========== */}
                          <p className=" transition duration-700 px-6 font-bold mb-2">
                            {cert.title}
                          </p>
                        ...
                        {/* =========== TOOLTIP TEXT =========== */}
                        {/* <span className="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible z-50 py-1 px-2 bg-white border dark:border-[#0B192C] dark:bg-[#0B192C] text-xs font-medium dark:text-white rounded shadow-sm" role="tooltip">
                          Open Certificate
                        </span> */}
                      </div>
                    </div>
          ...
          <div className="hs-carousel-pagination flex justify-center absolute bottom-3 start-0 end-0 space-x-2">
            {user_info.certificates.map((cert, index) => {
              return (
                <span
                  className="hs-carousel-active:bg-[#C96868] hs-carousel-active:border-[#C96868] size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-[#E78F81] dark:hs-carousel-active:border-[#E78F81]"
                  key={index}
                ></span>
              );
            })}
      ...
      <div className="w-full">
        {/* =========== EXPERIENCE TITLE =========== */}
        <h4 className="text-xl dark:text-white mb-4 font-bold flex gap-2 items-center">
          <FaBuildingUser className="text-2xl text-[#C96868] dark:text-[#E78F81]" />
          Experience
        </h4>
                      ...
                      {exp.descriptions.map((desc, index) => {
                        return (
                          <li className="flex space-x-3" key={index}>
                            <svg
                              className="flex-shrink-0 size-4 mt-0.5 text-[#C96868] dark:text-[#E78F81]"
                              xmlns="http://www.w3.org/2000/svg"
                              width="24"
                              height="24"
                              viewBox="0 0 24 24"
